...
export default function DynamicBadge(props) {
  ...
  React.useLayoutEffect(() => {
    ...
    if (items.length > 0) {
      ...
      rulerSpan.innerHTML =
        items.length === 1 ? items[0].substring(0, 3) + '...' : items[0].substring(0, 2) + ', ...'
    ...
    // If the items is only one, no need for badges
      ...
      items.forEach((item, index) => {
        ...
        const rulerSpanWidth = props.testRulerSpanWidth
          ? props.testRulerSpanWidth - (props.testRulerSpanWidth / items.length) * index
      ...
      <div style={{ width: '100%' }}>
        {state.badgeCounter > 0 ? (
          <div className="bdg-container">
            {!props.onlyBadge && (
              <div className="bdg-ellipsis">
                {state.itemsToDisplay === '' ? '' : state.itemsToDisplay + ', ...'}
              </div>
            )}
            <div
              className={props.badgeClass ? props.badgeClass : 'bdg-badge'}
              title={props.items && props.items.join(', ')}
            >
              {state.badgeCounter}
            </div>
          </div>
        ) : (
          <div className="bdg-ellipsis" style={{ width: '100%' }}>
